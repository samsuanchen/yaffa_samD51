...
/** binary.                                                                  **/
/******************************************************************************/
uint8_t isDLNumber(char* subString) {
...
SKIP:                // common code to skip initial character
  ...
  while (*subString) {
    char* pos = strchr(dl_charset, (int)tolower(*subString));
    cell_t offset = pos - dl_charset;
    if ((offset < base) && (offset > -1))
      number = (number * base) + (pos - dl_charset);
    else {
      base = tempBase;
